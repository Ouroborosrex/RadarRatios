...
class Transformer(nn.Module):
    ...
    def forward(self, x):
        ...
        for attn, ff in self.layers:
            x, attn_map = attn(x)
            #print('Attention',attn_map)
            attention_maps.append(attn_map)
            x = x + x
            x = ff(x) + x
        #print('Transformer',len(attention_maps))
...
class MobileViTBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat((x, y), 1)
        x = self.conv4(x)
        #print('MobileViTBlock',len(attention_maps))
        return x, attention_maps
...
class MobileViT(nn.Module):
    ...
    def forward(self, x):
        ...
        for conv, attn in self.trunk:
            x = conv(x)
            x, attention_maps = attn(x)
            attention_maps_all.extend(attention_maps)
        logits = self.to_logits(x)
        return logits, attention_maps_all